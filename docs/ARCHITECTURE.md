...
- `puppet_forge.voice`: local PuppetVoice articulated unit-bank synthesis for demo words with phrase prosody, unit-word coarticulation, source/filter fallback, normalization, G2P rules, engine versioning, phoneme timing, word cues, and visemes.
...
4. Local PuppetVoice synthesis uses generated word units for core puppet demo words, applies phrase-level stress and smoother word-to-word flow, falls back to source/filter phonemes for other words, then creates WAV audio, line cues, word cues, phoneme cues, and viseme timings.
