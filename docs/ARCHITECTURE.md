...
- `puppet_forge.voice`: local PuppetVoice articulated synthesis with continuous unit-word articulation, continuous fallback-word articulation, phrase prosody, word coarticulation, normalization, English-like G2P rules, engine versioning, phoneme timing, word cues, and visemes.
...
4. Local PuppetVoice synthesis uses generated word units for core puppet demo words, applies continuous fallback-word rendering and English-like pronunciation rules for other words, then creates WAV audio, line cues, word cues, phoneme cues, and viseme timings.
