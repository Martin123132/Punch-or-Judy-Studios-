...
- `puppet_forge.voice`: local PuppetVoice articulated unit-bank synthesis with source/filter fallback, normalization, G2P rules, engine versioning, phoneme timing, word cues, and visemes.
...
4. Local PuppetVoice synthesis uses generated word units for core clear-voice words, falls back to source/filter phonemes for other words, then creates WAV audio, line cues, word cues, phoneme cues, and viseme timings.
